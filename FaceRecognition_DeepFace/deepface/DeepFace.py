...
def represent_custom(
    ...
    for img, region, _ in img_objs:
        ...
        if type(img_path) is not numpy.ndarray:
            resp_obj["name"] = (os.path.splitext(os.path.basename(img_path))[0])
        resp_obj["embedding"] = embedding
        resp_obj["facial_area"] = region
        resp_objs.append(resp_obj)

    return resp_obj
def represent_custom2(
    img_path,
    img_name,
    model_name="VGG-Face",
    enforce_detection=True,
    ...
    for img, region, _ in img_objs:
        ...
        if (img_name) :
            resp_obj["name"] = (os.path.splitext(os.path.basename(img_name))[0])
        resp_obj["embedding"] = embedding
        resp_obj["facial_area"] = region
        resp_objs.append(resp_obj)
